...
public interface ICstKeyStatsService {
     ...
     */
    Map<String, Object> getSummaryAndDeptList(UseDeptScope scope);
     ...
     */
    List<Map<String, Object>> getDeptEquipList(String useDept, UseDeptScope scope);

    /**
     * 三级页：单设备按时段序列，groupBy=day|week|month|year
     * 每行: period, workHours, treatCount, totalCharge, avgChargePerCase
     */
    List<Map<String, Object>> getEquipSeries(Long equipId, String groupBy, String beginTime, String endTime, UseDeptScope scope);

    /**
     * 首页：全院重点设备今年 vs 去年 按月序列（收费与诊治例数）
     * 返回 key: thisYear / lastYear，value: List<Map> 每项 period(yyyy-MM), totalCharge, totalTreat
     */
    Map<String, List<Map<String, Object>>> getSummaryMonthlySeries(UseDeptScope scope);

    /**
     * 首页：重点设备今年收费 TOP N，返回 equipId/equipNo/equipDesc/useDept/totalCharge/totalTreat
     */
    List<Map<String, Object>> getKeyEquipTopByCharge(int limit, UseDeptScope scope);
     ...
     */
    Map<String, List<Map<String, Object>>> getTopEquipMonthlySeriesByValue(Long minValue, int limit, UseDeptScope scope);
     ...
     */
    List<Map<String, Object>> getTopEquipChargeYearCompareByValue(Long minValue, int limit, UseDeptScope scope);
}
