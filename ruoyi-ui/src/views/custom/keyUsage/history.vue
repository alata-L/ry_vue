  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="设备编号" align="center" prop="equipNo" min-width="100">
        <template slot-scope="scope">
          <span>{{ snapKey(scope.row).equipNo || '—' }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="工作时间" align="center" width="110">
        <template slot-scope="scope">
          <span>{{ fmtNum(snapKey(scope.row).workHours) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="每周工作天数" align="center" width="110">
        <template slot-scope="scope">
          <span>{{ snapKey(scope.row).weekWorkDays != null && snapKey(scope.row).weekWorkDays !== '' ? snapKey(scope.row).weekWorkDays : '—' }}</span>
        </template>
      </el-table-column>
      <el-table-column label="日均服务例数" align="center" width="110">
        <template slot-scope="scope">
          <span>{{ snapKey(scope.row).treatCount != null && snapKey(scope.row).treatCount !== '' ? snapKey(scope.row).treatCount : '—' }}</span>
        </template>
      </el-table-column>
      <el-table-column label="收费价格" align="center" width="100">
        <template slot-scope="scope">
          <span>{{ fmtNum(snapKey(scope.row).unitChargePrice) }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="变更明细" :visible.sync="detailOpen" width="820px" append-to-body custom-class="hist-detail-dialog" @closed="detailRow = null">
      <div v-if="detailRow" class="hist-detail-wrap">
        ...
        <el-table :data="keyDetailCompareRows" border size="small" class="hist-compare-table" :row-class-name="keyDetailRowClass">
        ...
        <el-collapse v-if="keyDetailRawFallback" class="hist-raw-collapse">
...
<script>
import { listKeyUsageHistory } from '@/api/custom/keyUsage'

export default {
  name: 'KeyUsageHistory',
  dicts: ['cst_use_dept'],
  data() {
    return {
      loading: true,
  ...
  computed: {
    keyDetailCompareRows() {
      if (!this.detailRow) {
        return []
      }
      return this.buildKeyCompareRows(this.detailRow.beforeJson, this.detailRow.afterJson)
    },
    keyDetailRawFallback() {
      return this.detailRow && this.keyDetailCompareRows.length === 0
    }
  },
  watch: {
    '$route'(to) {
      if (!to.path.includes('keyUsageHistory')) {
        return
      }
  ...
  methods: {
    ...
    getList() {
      ...
      listKeyUsageHistory(this.addDateRange(p, this.dateRange))
        .then(res => {
          this.list = res.rows
    ...
    keyDetailRowClass({ row }) {
      return row.diff ? 'hist-diff-row' : ''
    },
    buildKeyCompareRows(beforeJson, afterJson) {
      ...
      const order = ['reportDate', 'equipNo', 'useDept', 'treatCount', 'workHours', 'weekWorkDays', 'unitChargePrice', 'remark', 'reportBy']
      const labelMap = {
        reportDate: '上报日期',
        equipNo: '设备编号',
        useDept: '使用科室',
        ...
        workHours: '工作时间',
        weekWorkDays: '每周工作天数',
        unitChargePrice: '收费价格',
        remark: '备注',
        reportBy: '上报人（登录名）'
      }
      const sorted = [...keys].filter(k => k !== '__proto__' && k !== 'id' && k !== 'equipId' && k !== 'equipDesc').sort((x, y) => {
    ...
    formatKeyValue(key, val) {
      ...
      if (key === 'unitChargePrice' || key === 'workHours') {
        return String(val)
      }
      return String(val)
    },
