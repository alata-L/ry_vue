...
export default {
  ...
  data() {
    return {
      ...
      /** ≥100万：TOP10 设备今年/去年（收费、工作时长、诊疗例数共用同一批数据） */
      compare100: [],
      chartCharge100: null,
  ...
  beforeDestroy() {
    const charts = [this.chartCharge100, this.chartWorkHours100, this.chartTreat100]
    charts.forEach(ch => {
      if (ch) {
        ch.dispose()
      }
    })
    this.chartCharge100 = this.chartWorkHours100 = this.chartTreat100 = null
  },
  methods: {
    resize() {
      const charts = [this.chartCharge100, this.chartWorkHours100, this.chartTreat100]
    ...
    loadData() {
      Promise.allSettled([
        ...
        if (r1.status === 'fulfilled' && r1.value.data) {
          this.compare100 = Array.isArray(r1.value.data) ? r1.value.data : []
        }
